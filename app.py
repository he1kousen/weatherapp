...
class Weather(Tk):
    ...
    def __gui(self):
       # menaruh border hitam pada kolom search
       ...
       # label cuaca saat ini untuk menampilkan city_name dan city_time
       Label(text="Cuaca Saat Ini :",font='Arial 14 bold',fg="red").place(x=590,y=7)

       # logo foto location/lokasi
       ...
       # placing the labels
       Label(text="Kelembaban",font="Calibri 15 bold",bg='#00b7ff',fg='white').place(x=35,y=335)
       Label(text="Tekanan",font="Calibri 15 bold",bg='#00b7ff',fg='white').place(x=210,y=335)
       Label(text="Deskripsi",font="Calibri 15 bold",bg='#00b7ff',fg='white').place(x=400,y=335)
       Label(text="Jarak Pandang",font="Calibri 15 bold",bg='#00b7ff',fg='white').place(x=600,y=335)
    ...
    def __get_weather(self):
        try:
            ...
            data=f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid={api}&lang=id'
    ...
    def __set_information(self,weather):
        ...
        if weather['cod']=='404' and weather['message']=='city not found':
            messagebox.showerror("Error","Kota tersebut tidak ada!")
            self.search.set("")
        elif weather['cod']=='400' and weather['message']=='Nothing to geocode':
            messagebox.showinfo("Warning",'Masukkan nama Kota-nya!')
            self.search.set('')
        else:
            ...
            self.feel['text']=f"Terasa seperti {int(weather['main']['feels_like']-273)}° | {weather['weather'][0]['main']}"
    ...
    def exit(self):
        a=messagebox.askyesno('Konfirmasi',"Apakah anda yakin ingin keluar?")
        if a==True:
            self.destroy()
